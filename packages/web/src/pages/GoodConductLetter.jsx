...
import { addLetter, updateLetter, getAllStudents } from '../services/dataService';
...
const GoodConductLetter = () => {
    ...
    // Data Siswa (Fetch dari database/dataService)
    ...
    const [formData, setFormData] = useState({
        ...
        nisn: '',
        kelas: '',
        nama_orang_tua: '', // <-- Field Nama Orang Tua
        alamat: '',
        keperluan: 'Melanjutkan sekolah / Persyaratan pendaftaran',
    ...
    const handleSearch = (e) => {
        ...
        if (term.length > 0) {
            const filtered = students.filter(std =>
                std.name?.toLowerCase().includes(term.toLowerCase()) ||
                std.nisn?.includes(term)
            );
    ...
    // LOGIKA AUTO-FILL (Sesuai Skema StudentData)
    const handleSelectStudent = (student) => {
        // Menyusun alamat detail otomatis jika data tesedia di database
        const alamatLengkap = [
            student.address,
            ...
            student.city ? `Kab. ${student.city}` : ''
        ].filter(Boolean).join(', ');

        setFormData(prev => ({
            ...prev,
            nama_siswa: student.name || '',
            tempat_lahir: student.placeOfBirth || student.tempat_lahir || '',
            tanggal_lahir: student.dateOfBirth || student.tanggal_lahir || '',
            nisn: student.nisn || '',
            kelas: student.groupName || student.kelas || '',
            nama_orang_tua: student.parentName || student.nama_orang_tua || '',
            alamat: alamatLengkap || student.alamat || ''
        }));
    ...
    const saveLetter = async (e, status = 'sent') => {
        ...
        try {
            const letterData = {
                letterType: 'kelakuan_baik',
                letterNumber: formData.nomor_surat,
                subject: `Surat Kelakuan Baik - ${formData.nama_siswa}`,
                formData: formData,
                status: status,
        ...
        <div className="flex-1 overflow-y-auto z-10 p-6 md:p-10 scroll-smooth relative">
            ...
            <div className="max-w-[960px] mx-auto flex flex-col gap-8">
                ...
                <div className="flex flex-wrap gap-2 px-1">
                    ...
                    <span className="text-text-secondary text-sm font-medium">/</span>
                    <span className="text-white text-sm font-medium">Kelakuan Baik</span>
                </div>

                {/* Header */}
                <div className="flex flex-col gap-2">
                    <h1 className="text-white text-3xl sm:text-4xl font-black leading-tight tracking-tight">
                        {editMode ? 'Edit' : 'Formulir'} Surat Kelakuan Baik
                    </h1>
                    <p className="text-text-secondary text-base font-normal">
                        Buat surat keterangan berkelakuan baik untuk keperluan siswa (melanjutkan sekolah, beasiswa, dll).
                    </p>
                </div>
                    ...
                    {showSuggestions && (
                        <div className="absolute left-6 right-6 top-[calc(100%-10px)] z-50 bg-[#1c1b2e] border border-[#383663] rounded-b-xl shadow-2xl max-h-60 overflow-y-auto custom-scrollbar">
                            {filteredStudents.length > 0 ? (
                                ...
                                <div className="px-4 py-3 text-[#9795c6] text-sm">Data siswa tidak ditemukan</div>
                            )}
                        </div>
                    )}
                </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="relative group">
                                    <Flatpickr
                                        ...
                                        onChange={([date], dateStr) => handleDateChange('tanggal_lahir', dateStr)}
                                        options={{
                                            locale: Indonesian,
                                            dateFormat: "Y-m-d",
                                            altInput: true,
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="relative">
                                    ...
                                    <span className="material-symbols-outlined absolute right-4 top-3 text-[#9795c6] pointer-events-none text-[20px]">pin</span>
                                </div>
                            </div>
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="relative group">
                                    <select
                                        ...
                                        onChange={handleInputChange}
                                        className="w-full bg-[#131221] text-white rounded-xl border border-[#272546] group-hover:border-primary/60 focus:border-primary focus:ring-2 focus:ring-primary/20 h-12 px-4 transition-all duration-300 ease-in-out appearance-none cursor-pointer shadow-sm group-hover:shadow-[0_0_10px_rgba(99,102,241,0.1)]"
                                        required
                                    >
                                        <option disabled value="">Pilih Tingkat Kelas</option>
                                        <option value="7">Kelas 7</option>
                                        <option value="8">Kelas 8</option>
                                        <option value="9">Kelas 9</option>
                                    </select>
                                    <span className="material-symbols-outlined absolute right-4 top-3 text-[#9795c6] group-hover:text-primary transition-colors duration-300 pointer-events-none">
                                        expand_more
                                    </span>
                                </div>
                            </div>
                            ...
                            <div className="md:col-span-2 space-y-2">
                                ...
                                <div className="relative">
                                    <input
                                        ...
                                        required
                                    />
                                    <span className="material-symbols-outlined absolute right-4 top-3 text-[#9795c6] pointer-events-none text-[20px]">family_restroom</span>
                                </div>
                            </div>

                            <div className="md:col-span-2 space-y-2">
                                <label className="text-text-secondary text-sm font-medium ml-1">Alamat Lengkap</label>
                                <textarea name="alamat" value={formData.alamat} onChange={handleInputChange} className="w-full bg-[#131221] text-white rounded-xl border border-[#272546] focus:border-primary focus:ring-1 focus:ring-primary min-h-[100px] p-4 resize-none transition-all" placeholder="Alamat domisili siswa..."></textarea>
                            </div>
                        </div>
                    </div>
...
export default GoodConductLetter;