...
import { getAllLetters, addLetter, updateLetter, deleteLetter } from '../services/dataService';
import { generateAssignmentLetter, generateActiveStudentLetter } from '../utils/documentGenerator';
import PrintSiswaAktif from '../components/print/PrintSiswaAktif';
import PrintSuratTugas from '../components/print/PrintSuratTugas';
import PrintMutasiMasuk from '../components/print/PrintMutasiMasuk';
import PrintKelakuanBaik from '../components/print/PrintKelakuanBaik';
import PrintSPPD from '../components/print/PrintSPPD';
import PrintPanggilanOrangTua from '../components/print/PrintPanggilanOrangTua';
import PrintPinjamBMN from '../components/print/PrintPinjamBMN';
...
const LetterHistory = () => {
    ...
    const [filterTab, setFilterTab] = useState('all'); // all, sent, draft
    ...
    useEffect(() => {
        ...
        if (tabParam && ['all', 'sent', 'draft'].includes(tabParam)) {
    ...
    const fetchLetters = async () => {
        ...
        try {
            ...
            showToast("Gagal memuat riwayat surat", "error");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (modalMode === 'add') {
                ...
                showToast("Riwayat surat berhasil ditambahkan", "success");
            } else if (modalMode === 'edit') {
                await updateLetter(currentLetterId, formData);
                showToast("Riwayat surat berhasil diperbarui", "success");
            }
            closeModal();
            fetchLetters();
        } catch (error) {
            showToast(`Gagal ${modalMode === 'add' ? 'menambahkan' : 'mengupdate'} surat: ` + error.message, "error");
        } finally {
    ...
    const handleConfirmDelete = async () => {
        setIsSubmitting(true);
        try {
            await deleteLetter(deleteModal.letterId);
            showToast("Surat berhasil dihapus", "success");
            fetchLetters();
        } catch (error) {
            showToast("Gagal menghapus surat: " + error.message, "error");
        } finally {
            setIsSubmitting(false);
    ...
    const handleEditLetter = (letter) => {
        // Navigate to the appropriate form based on letterType
        if (letter.letterType === 'surat_tugas') {
            navigate('/layanan-surat/surat-tugas', { state: { letter } });
        } else if (letter.letterType === 'surat_keterangan_siswa_aktif') {
            navigate('/layanan-surat/keterangan-aktif', { state: { letter } });
        } else if (letter.letterType === 'mutasi_masuk') {
            navigate('/layanan-surat/mutasi-masuk', { state: { letter } });
        } else if (letter.letterType === 'kelakuan_baik') {
            navigate('/layanan-surat/keterangan-baik', { state: { letter } });
        } else if (letter.letterType === 'sppd') {
            navigate('/layanan-surat/sppd', { state: { letter } });
        } else if (letter.letterType === 'panggilan_orang_tua') {
            navigate('/layanan-surat/panggilan-orang-tua', { state: { letter } });
        } else if (letter.letterType === 'izin_pinjam_bmn') {
            navigate('/layanan-surat/izin-pinjam-bmn', { state: { letter } });
        } else {
            // Default or other letter types can be handled here
            showToast("Tipe surat ini belum mendukung edit", "info");
        }
    };

    const handleDownloadWord = async (letter) => {
        showToast("Sedang mengunduh dokumen Word...", "info");
        try {
            // Try backend API first
            const API_URL = import.meta.env.VITE_API_URL || 'http://localhost:3001';
            let endpoint = `/api/letters/${letter.id}/download-word`;
            if (letter.letterType === 'surat_keterangan_siswa_aktif') {
                endpoint = `/api/letters/${letter.id}/generate-siswa-aktif/word`;
            } else if (letter.letterType === 'surat_tugas') {
                endpoint = `/api/letters/${letter.id}/generate-surat-tugas/word`;
            }
            ...
            if (response.ok) {
                ...
                a.href = url;
                a.download = `Surat_${letter.letterNumber || letter.id}.docx`;
                ...
                if (letter.formData) {
                    if (letter.letterType === 'surat_keterangan_siswa_aktif') {
                        await generateActiveStudentLetter(letter.formData);
                    } else {
                        await generateAssignmentLetter(letter.formData);
                    }
                    showToast("Dokumen Word berhasil diunduh (client-side)", "success");
                } else {
                    showToast("Data formulir tidak tersedia", "error");
                }
            }
        } catch (error) {
            ...
            try {
                if (letter.formData) {
                    ...
                    showToast("Dokumen Word berhasil diunduh (fallback)", "success");
                } else {
                    showToast("Gagal mengunduh: " + error.message, "error");
                }
            } catch (fallbackError) {
                showToast("Gagal mengunduh: " + fallbackError.message, "error");
            }
        }
    };

    const handlePrint = (letter) => {
        // Ensure letterType is preserved even if we use formData
        const data = letter.formData ? { ...letter.formData, letterType: letter.letterType } : letter;
        setPrintData(data);
        // Give React time to render the component before calling print
        setTimeout(() => {
            window.print();
            // Clear print data after a short delay to allow browser to render
    ...
    const handleExportCSV = () => {
        const headers = ["Agenda", "No. Surat", "Tanggal", "Jenis", "Pengirim", "Penerima", "Perihal", "Klasifikasi"];
        ...
        link.setAttribute("download", `riwayat_surat_${filterTab}.csv`);
    ...
    const filteredData = letters.filter(letter => {
        ...
        let matchesTab = true;
        if (filterTab === 'sent') matchesTab = letter.status === 'sent';
        if (filterTab === 'draft') matchesTab = letter.status === 'draft';
    ...
    // --- UI Helpers ---
    const getTypeColor = (type) => type === 'incoming'
    ...
    return (
        <div className="flex-1 overflow-y-auto p-6 md:p-8 relative">
            ...
            <ConfirmationModal
                ...
                title="Hapus Surat"
                message={`Apakah Anda yakin ingin menghapus surat "${deleteModal.letterTitle}"?`}
            ...
            <div className={`max-w-[1400px] mx-auto flex flex-col gap-6 ${isModalOpen ? 'blur-sm' : ''}`}>
                ...
                <div className="flex flex-col gap-2">
                    <h1 className="text-3xl md:text-4xl font-black text-white tracking-tight">Riwayat Surat</h1>
                    <p className="text-[#9795c6]">Log otomatis surat yang dibuat oleh sistem.</p>
                </div>
                    ...
                    <div className="flex flex-col xl:flex-row gap-4 justify-between items-start xl:items-center">
                        ...
                        <div className="flex gap-1 p-1 bg-[#1c1b2e] rounded-xl border border-[#272546] w-full xl:w-auto overflow-x-auto">
                            {['all', 'sent', 'draft'].map(tab => (
                                ...
                                >
                                    {tab === 'all' ? 'Semua' : tab === 'sent' ? 'Surat Keluar' : 'Draft'}
                                </button>
                            ))}
                        </div>
                ...
                <div className="glass-panel rounded-2xl border border-[#272546] overflow-hidden flex-1 flex flex-col min-h-[400px]">
                    {loading ? (
                        <div className="flex flex-1 items-center justify-center flex-col gap-4">
                            ...
                            <p className="text-[#9795c6]">Memuat data surat...</p>
                        </div>
                    ) : filteredData.length === 0 ? (
                        <div className="flex flex-1 items-center justify-center flex-col gap-4 p-10 text-center">
                            <span className="material-symbols-outlined text-6xl text-[#272546]">inbox</span>
                            <div className="text-[#9795c6]">Belum ada data surat ditemukan.</div>
                        </div>
                    ) : (
                            ...
                            <table className="min-w-full text-left">
                                <thead className="bg-[#1c1b2e] border-b border-[#272546]">
                                    <tr>
                                        ...
                                        <th className="px-6 py-4 text-xs font-semibold text-[#9795c6] uppercase">Tipe</th>
                                        <th className="px-6 py-4 text-xs font-semibold text-[#9795c6] uppercase">Perihal</th>
                                        <th className="px-6 py-4 text-xs font-semibold text-[#9795c6] uppercase">Status</th>
                                ...
                                <tbody className="divide-y divide-[#272546]">
                                    {paginatedData.map((letter) => (
                                        <tr key={letter.id} className="hover:bg-[#272546]/30 transition-colors">
                                            ...
                                            <td className="px-6 py-4 text-sm text-white font-medium font-mono">{letter.letterNumber || letter.referenceNumber || '-'}</td>
                                            <td className="px-6 py-4">
                                                <span className="px-2 py-0.5 rounded text-xs border bg-indigo-500/10 text-indigo-400 border-indigo-500/20">
                                                    {letter.letterType === 'surat_tugas' ? 'Surat Tugas' : letter.letterType === 'surat_keterangan_siswa_aktif' ? 'Surat Keterangan Siswa Aktif' : letter.letterType === 'mutasi_masuk' ? 'Surat Mutasi Masuk' : letter.letterType === 'kelakuan_baik' ? 'Surat Kelakuan Baik' : letter.letterType === 'sppd' ? 'Surat Perjalanan Dinas' : letter.letterType || 'Umum'}
                                                </span>
                                            </td>
                                            <td className="px-6 py-4 text-sm text-[#9795c6] max-w-xs truncate" title={letter.subject}>{letter.subject}</td>
                                            <td className="px-6 py-4">
                                                <span className={`px-2 py-0.5 rounded text-xs border ${letter.status === 'draft' ? 'bg-amber-500/10 text-amber-400 border-amber-500/20' :
                                                    letter.status === 'sent' ? 'bg-green-500/10 text-green-400 border-green-500/20' :
                                                        'bg-blue-500/10 text-blue-400 border-blue-500/20'
                                                    }`}>
                                                    {letter.status === 'draft' ? 'Draft' : letter.status === 'sent' ? 'Terkirim' : letter.status}
                                                </span>
                                            </td>
                                            <td className="px-6 py-4 text-right">
                                                <div className="flex justify-end gap-1">
                                                    <button
                                                        ...
                                                        className="p-1.5 hover:bg-amber-500/10 text-[#9795c6] hover:text-amber-400 rounded-lg transition-colors"
                                                        title="Edit"
                                                    >
                                                    ...
                                                    <button
                                                        ...
                                                        className="p-1.5 hover:bg-red-500/10 text-[#9795c6] hover:text-red-400 rounded-lg transition-colors"
                                                        title="Hapus"
                                                    >
                                                    ...
                                                    <button
                                                        ...
                                                        className="p-1.5 hover:bg-green-500/10 text-[#9795c6] hover:text-green-400 rounded-lg transition-colors"
                                                        title="Download Word"
                                                    >
                                                    ...
                                                    <button
                                                        ...
                                                        className="p-1.5 hover:bg-blue-500/10 text-[#9795c6] hover:text-blue-400 rounded-lg transition-colors"
                                                        title="Cetak Surat"
                                                    >
                    ...
                    <div className="relative bg-[#1c1b2e] border border-[#272546] rounded-2xl w-full max-w-2xl shadow-2xl flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200">
                        <div className="px-6 py-4 border-b border-[#272546] flex items-center justify-between flex-shrink-0">
                            <h3 className="text-xl font-bold text-white">
                                {modalMode === 'add' ? 'Tambah Riwayat Surat' : modalMode === 'edit' ? 'Edit Riwayat Surat' : 'Detail Surat'}
                            </h3>
                        ...
                        <div className="p-6 overflow-y-auto custom-scrollbar flex-1">
                            {modalMode === 'detail' ? (
                                ...
                                <form id="letterForm" onSubmit={handleSubmit} className="flex flex-col gap-4">
                                    ...
                                    <div className="flex flex-col gap-1.5">
                                        <label className="text-xs font-semibold text-[#9795c6] uppercase">Status</label>
                                        <select className="input-field bg-[#131221]" value={formData.status} onChange={e => setFormData({ ...formData, status: e.target.value })}>
                        ...
                        <div className="p-6 border-t border-[#272546] flex justify-end gap-3 flex-shrink-0 bg-[#1c1b2e]">
                            ...
                            {modalMode !== 'detail' && (
                                <button form="letterForm" type="submit" disabled={isSubmitting} className="px-6 py-2 rounded-xl bg-primary hover:bg-primary/90 text-white shadow-lg shadow-primary/25 transition-all font-semibold text-sm disabled:opacity-50">
            ...
            {printData?.letterType === 'surat_keterangan_siswa_aktif' && <PrintSiswaAktif data={printData} />}
            {printData?.letterType === 'surat_tugas' && <PrintSuratTugas data={printData} />}
            {printData?.letterType === 'mutasi_masuk' && <PrintMutasiMasuk data={printData} />}
            {printData?.letterType === 'kelakuan_baik' && <PrintKelakuanBaik data={printData} />}
            {printData?.letterType === 'sppd' && <PrintSPPD data={printData} />}
            {printData?.letterType === 'panggilan_orang_tua' && <PrintPanggilanOrangTua data={printData} />}
            {printData?.letterType === 'izin_pinjam_bmn' && <PrintPinjamBMN data={printData} />}
        </div>
    );
};
